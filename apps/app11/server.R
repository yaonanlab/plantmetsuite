...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app11_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app11_inputFile1$datapath
    ...
    if(!is.null(input$app11_email)&(input$app11_email)!=""){
	app11_email <- as.character(input$app11_email)
	}else{
	app11_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app11",mytaskid=taskidnow,myemail=app11_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app11_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app11/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
