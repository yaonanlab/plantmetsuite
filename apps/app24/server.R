...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app24_inputFile1)&!is.null(input$app24_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app24_inputFile1$datapath
	...
	temp.file2 <- input$app24_inputFile2$datapath
    ...
    if(!is.null(input$app24_email)&(input$app24_email)!=""){
	app24_email <- as.character(input$app24_email)
	}else{
	app24_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app24",mytaskid=taskidnow,myemail=app24_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app24_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app24/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
