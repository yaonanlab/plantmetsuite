...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app18_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app18_inputFile1$datapath
    ...
    if(!is.null(input$app18_email)&(input$app18_email)!=""){
	app18_email <- as.character(input$app18_email)
	}else{
	app18_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app18",mytaskid=taskidnow,myemail=app18_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app18_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app18/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
