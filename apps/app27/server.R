...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app27_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app27_inputFile1$datapath
	...
	app27_select1 <- as.character(input$app27_select1)
	app27_select2 <- as.character(input$app27_select2)
    if(!is.null(input$app27_email)&(input$app27_email)!=""){
	app27_email <- as.character(input$app27_email)
	}else{
	app27_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app27",mytaskid=taskidnow,myemail=app27_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app27_select1,app27_select2,app27_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app27/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
