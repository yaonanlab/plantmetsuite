...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app2_inputFile1)&!is.null(input$app2_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app2_inputFile1$datapath
	...
	temp.file2 <- input$app2_inputFile2$datapath
    ...
    app2_select1 <- as.character(input$app2_select1)
    if(!is.null(input$app2_email)&(input$app2_email)!=""){
	app2_email <- as.character(input$app2_email)
	}else{
	app2_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app2",mytaskid=taskidnow,myemail=app2_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app2_select1,app2_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app2/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
