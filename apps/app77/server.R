...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app77_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app77_inputFile1$datapath
    ...
    if(!is.null(input$app77_email)&(input$app77_email)!=""){
	app77_email <- as.character(input$app77_email)
	}else{
	app77_email <-"no"
	}
	...
	ipresult<-data.frame(myip=ip,myapp="app77",mytaskid=taskidnow,myemail=app77_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}

mycmdfirst<-paste(taskidnow,app77_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app77/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
