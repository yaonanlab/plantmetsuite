...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app22_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app22_inputFile1$datapath
    ...
    if(!is.null(input$app22_email)&(input$app22_email)!=""){
	app22_email <- as.character(input$app22_email)
	}else{
	app22_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app22",mytaskid=taskidnow,myemail=app22_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app22_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app22/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
