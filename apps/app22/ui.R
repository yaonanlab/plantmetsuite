appUI<-function(input,output){
    ...
    div(
      ...
      tabBox(
        ...
        height = NULL,
        tabPanel(
          title = img(src = 'app22.png', height = '40px', width = '40px',style="display: inline;"),
          div(
            style = "position: absolute; left: 0.5em; bottom: 0.5em;display:inline;"
          ),div(h3(strong("GFF3转GTF")), h4("Convert GFF3 format files to GTF format files"),style="text-align: center;display:inline;",
		  h4("Required field",style="text-align: left;font-weight: 1000;color:red"),
		  div(id = "app22_fileupload",
                            tags$style("
                            .btn-file {
							...
							"),br(),h4("Select the data to be uploaded：gff3、gff、txt",style="text-align: left;font-weight: 1000;")
							,
							fluidRow(column(width=5,fileInput(
                                inputId="app22_inputFile1",
                                label = "",
                                accept = c('.gff3',
                                           '.gff',
                                           '.txt')
                 ...
                 style = "color: white; 
					   ...
					   vertical-align:middle;
					   "), target = "_blank",
                            href = "app22/genome.txt"),a(h4("Download sample data", class = "action-button", 
                 style = "color: white; 
                       background-color: #53d362; 
                       height: 40px;
                       width: 90px;
...
div(textAreaInput("app22_email", "","", width = "100%",height="40px"),style="text-align: center;font-weight: 1000;color:red"),
							actionButton(
                 inputId = "send_button",
                 label = "Submit",
                 styleclass = "info",
