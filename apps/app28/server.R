...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app28_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app28_inputFile1$datapath
	...
	app28_select1 <- as.character(input$app28_select1)
	app28_select2 <- as.character(input$app28_select2)
    if(!is.null(input$app28_email)&(input$app28_email)!=""){
	app28_email <- as.character(input$app28_email)
	}else{
	app28_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app28",mytaskid=taskidnow,myemail=app28_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app28_select1,app28_select2,app28_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app28/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
