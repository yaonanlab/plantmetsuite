...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app111_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app111_inputFile1$datapath
	...
	app111_text1 <- as.numeric(input$app111_text1)
    if(!is.null(input$app111_email)&(input$app111_email)!=""){
	app111_email <- as.character(input$app111_email)
	}else{
	app111_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app111",mytaskid=taskidnow,myemail=app111_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app111_text1,app111_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app111/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
