...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app23_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app23_inputFile1$datapath
    ...
    if(!is.null(input$app23_email)&(input$app23_email)!=""){
	app23_email <- as.character(input$app23_email)
	}else{
	app23_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app23",mytaskid=taskidnow,myemail=app23_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app23_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app23/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
