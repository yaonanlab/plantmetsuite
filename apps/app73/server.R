...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app73_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app73_inputFile1$datapath
	...
	if(!is.null(input$app73_email)&(input$app73_email)!=""){
	app73_email <- as.character(input$app73_email)
	}else{
	app73_email <-"no"}
	
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app73",mytaskid=taskidnow,myemail=app73_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app73_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app73/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
