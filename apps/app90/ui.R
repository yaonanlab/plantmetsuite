...
renderUI({
    div(
      ...
      tabBox(
        ...
        height = NULL,
        tabPanel(
          title = img(src = 'app90.png', height = '40px', width = '40px',style="display: inline;"),
          div(
            style = "position: absolute; left: 0.5em; bottom: 0.5em;display:inline;"
          ),div(h3(strong("Long table to wide")), h4("Converting long tables into wide tables for reading and analysis"),style="text-align: center;display:inline;",
		  h4("Required field",style="text-align: left;font-weight: 1000;color:red"),
		  div(id = "app90_fileupload",
                            tags$style("
                            .btn-file {
	...
	fluidRow(column(width=5,fileInput(
		...
		column(width=2,column(width=12,a(h4("View sample data", class = "action-button", 
                 style = "color: white; 
					   ...
					   vertical-align:middle;
					   "), target = "_blank",
                            href = "app90/data.txt"),a(h4("Download sample data", class = "action-button", 
                 style = "color: white; 
                       background-color: #53d362; 
                       height: 40px;
                       width: 90px;
...
div(textAreaInput("app90_email", "","", width = "100%",height="40px"),style="text-align: center;font-weight: 1000;color:red"),
