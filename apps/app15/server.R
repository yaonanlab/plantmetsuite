...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app15_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app15_inputFile1$datapath
	...
	app15_text1 <- as.numeric(input$app15_text1)
    if(!is.null(input$app15_email)&(input$app15_email)!=""){
	app15_email <- as.character(input$app15_email)
	}else{
	app15_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app15",mytaskid=taskidnow,myemail=app15_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app15_text1,app15_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app15/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
