...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app20_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app20_inputFile1$datapath
    ...
    if(!is.null(input$app20_email)&(input$app20_email)!=""){
	app20_email <- as.character(input$app20_email)
	}else{
	app20_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app20",mytaskid=taskidnow,myemail=app20_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app20_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app20/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
