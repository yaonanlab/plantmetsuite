...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app110_inputFile1)&!is.null(input$app110_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app110_inputFile1$datapath
	...
	temp.file2 <- input$app110_inputFile2$datapath
    ...
    app110_select1 <- as.character(input$app110_select1)
    if(!is.null(input$app110_email)&(input$app110_email)!=""){
	app110_email <- as.character(input$app110_email)
	}else{
	app110_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app110",mytaskid=taskidnow,myemail=app110_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app110_select1,app110_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app110/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
