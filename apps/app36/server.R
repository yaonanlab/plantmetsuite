...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app36_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app36_inputFile1$datapath
    ...
    if(!is.null(input$app36_email)&(input$app36_email)!=""){
	app36_email <- as.character(input$app36_email)
	}else{
	app36_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app36",mytaskid=taskidnow,myemail=app36_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app36_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app36/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
