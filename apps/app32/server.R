...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app32_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app32_inputFile1$datapath
	...
	app32_select1 <- as.character(input$app32_select1)
    if(!is.null(input$app32_email)&(input$app32_email)!=""){
	app32_email <- as.character(input$app32_email)
	}else{
	app32_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app32",mytaskid=taskidnow,myemail=app32_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app32_select1,app32_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app32/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
