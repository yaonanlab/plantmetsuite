...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app6_inputFile1)&!is.null(input$app6_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app6_inputFile1$datapath
	...
	temp.file2 <- input$app6_inputFile2$datapath
    ...
    app6_select1 <- as.character(input$app6_select1)
    if(!is.null(input$app6_email)&(input$app6_email)!=""){
	app6_email <- as.character(input$app6_email)
	}else{
	app6_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app6",mytaskid=taskidnow,myemail=app6_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app6_select1,app6_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app6/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
