...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app13_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app13_inputFile1$datapath
    ...
    if(!is.null(input$app13_email)&(input$app13_email)!=""){
	app13_email <- as.character(input$app13_email)
	}else{
	app13_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app13",mytaskid=taskidnow,myemail=app13_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app13_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app13/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
