...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app19_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app19_inputFile1$datapath
    ...
    if(!is.null(input$app19_email)&(input$app19_email)!=""){
	app19_email <- as.character(input$app19_email)
	}else{
	app19_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app19",mytaskid=taskidnow,myemail=app19_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app19_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app19/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
