...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app14_inputFile1)&!is.null(input$app14_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app14_inputFile1$datapath
	...
	temp.file2 <- input$app14_inputFile2$datapath
    ...
    if(!is.null(input$app14_email)&(input$app14_email)!=""){
	app14_email <- as.character(input$app14_email)
	}else{
	app14_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app14",mytaskid=taskidnow,myemail=app14_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app14_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app14/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
