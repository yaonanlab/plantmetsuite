appUI<-function(input,output){
    ...
    div(
      ...
      tabBox(
        ...
        height = NULL,
        tabPanel(
          title = img(src = 'app115.png', height = '40px', width = '40px',style="display: inline;"),
          div(
            style = "position: absolute; left: 0.5em; bottom: 0.5em;display:inline;"
          ),div(h3(strong("Fisher'sTest")), h4("Fisher'sTest"),style="text-align: center;display:inline;",
		h4("Required fields",style="text-align: left;font-weight: 1000;color:red"),
		  div(id = "app115_fileupload",
                            tags$style("
                            .btn-file {
		...
		h4("Select the table data to be examined：txt(tabular separator)、csv、xls、xlsx",style="text-align: left;font-weight: 1000;")
                 ...
                 style = "color: white; 
					   ...
					   vertical-align:middle;
					   "), target = "_blank",
                            href = "app115/data.txt"),a(h4("Get sample data", class = "action-button", 
                 style = "color: white; 
                       background-color: #53d362; 
                       height: 40px;
                       width: 90px;
...
div(textAreaInput("app115_email", "","", width = "100%",height="40px"),style="text-align: center;font-weight: 1000;color:red"),
							actionButton(
                 inputId = "send_button",
                 label = "Submit",
                 styleclass = "info",
