...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app29_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app29_inputFile1$datapath
	...
	app29_select1 <- as.character(input$app29_select1)
    if(!is.null(input$app29_email)&(input$app29_email)!=""){
	app29_email <- as.character(input$app29_email)
	}else{
	app29_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app29",mytaskid=taskidnow,myemail=app29_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app29_select1,app29_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app29/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
