...
job.path <- reactiveValues(data = NULL)
    ...
    if(!is.null(input$app108_inputFile)&done$data=="no"){
	load("userdata/jobid")
	...
	temp.file <- input$app108_inputFile$datapath
	...
	app108_text1<-as.character(input$app108_text1)
	app108_text2<-as.character(input$app108_text2)
	app108_text3<-as.character(input$app108_text3)
	app108_text4<-as.character(input$app108_text4)
	app108_text5<-as.character(input$app108_text5)
	app108_text6<-as.numeric(as.character((input$app108_text6)))
	app108_text7<-as.numeric(as.character((input$app108_text7)))	
    if(!is.null(input$app108_email)&(input$app108_email)!=""){
	app108_email <- as.character(input$app108_email)
	}else{
	app108_email <-"no"
	}
		ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app108",mytaskid=taskidnow,myemail=app108_email)
...
v <- venn.diagram(list(A=a, B=b),cat.pos = 0,fill = c(app108_text1, app108_text2),alpha = c(0.5, 0.5), cat.cex = app108_text6, cex=app108_text7,filename=NULL,category.names=names(data),scale=F)
grid.newpage()
grid.draw(v)
}
...
mycmdfirst<-paste(taskidnow,app108_text1,app108_text2,app108_text3,app108_text4,app108_text5,app108_text6,app108_text7,app108_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app108/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
