...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app78_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app78_inputFile1$datapath
	...
	app78_select1 <- as.numeric(input$app78_select1)
	if(!is.null(input$app78_select1)&(input$app78_select1)!=""){
	app78_select1 <- as.numeric(input$app78_select1)
	}else{
	app78_select1 <-1
	}
    ...
    if(!is.null(input$app78_email)&(input$app78_email)!=""){
	app78_email <- as.character(input$app78_email)
	}else{
	app78_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app78",mytaskid=taskidnow,myemail=app78_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app78_select1,app78_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app78/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
