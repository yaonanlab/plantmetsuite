...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app3_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app3_inputFile1$datapath
    ...
    if(!is.null(input$app3_email)&(input$app3_email)!=""){
	app3_email <- as.character(input$app3_email)
	}else{
	app3_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app3",mytaskid=taskidnow,myemail=app3_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app3_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app3/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
