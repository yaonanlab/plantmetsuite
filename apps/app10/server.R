...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app10_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app10_inputFile1$datapath
	...
	app10_text1 <- as.numeric(input$app10_text1)
	app10_text2 <- as.numeric(input$app10_text2)
	app10_select1 <- as.character(input$app10_select1)
    if(!is.null(input$app10_email)&(input$app10_email)!=""){
	app10_email <- as.character(input$app10_email)
	}else{
	app10_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app10",mytaskid=taskidnow,myemail=app10_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app10_text1,app10_text2,app10_select1,app10_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app10/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
