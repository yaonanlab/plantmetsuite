...
app93_select1<-as.character(args[2])
app93_select2<-as.character(args[3])
app93_email<-as.character(args[4])

# setwd(file.path("/srv/myapps/keyandaydayup/userdata",taskidnow))
parameter <-massconverter::create_msconvert_parameter(
    output_format = app93_select1,
...
if(app93_email!=""&("TRUE"%in%grepl("@",app93_email))){
setwd("/srv/myapps/keyandaydayup/")
tar(file.path("userdata",taskidnow,"result.zip"),files=file.path("userdata",taskidnow,"result"))
attach.file <- file.path("/srv/myapps/keyandaydayup/userdata",taskidnow,"result.zip")
sendmail(to=as.character(app93_email),attach=T,attach.file=attach.file)
}
