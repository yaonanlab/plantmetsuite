...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app21_inputFile1)&!is.null(input$app21_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app21_inputFile1$datapath
	...
	temp.file2 <- input$app21_inputFile2$datapath
    ...
    if(!is.null(input$app21_email)&(input$app21_email)!=""){
	app21_email <- as.character(input$app21_email)
	}else{
	app21_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app21",mytaskid=taskidnow,myemail=app21_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app21_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app21/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
