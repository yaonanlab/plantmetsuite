...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app9_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app9_inputFile1$datapath
    ...
    if(!is.null(input$app9_email)&(input$app9_email)!=""){
	app9_email <- as.character(input$app9_email)
	}else{
	app9_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app9",mytaskid=taskidnow,myemail=app9_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app9_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app9/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
