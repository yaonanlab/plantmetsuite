...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app7_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app7_inputFile1$datapath
    ...
    if(!is.null(input$app7_email)&(input$app7_email)!=""){
	app7_email <- as.character(input$app7_email)
	}else{
	app7_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app7",mytaskid=taskidnow,myemail=app7_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app7_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app7/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
