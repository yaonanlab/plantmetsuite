appUI<-function(input,output){
    ...
    div(
      ...
      tabBox(
        ...
        height = NULL,
        tabPanel(
          title = img(src = 'app7.png', height = '40px', width = '40px',style="display: inline;"),
          div(
            style = "position: absolute; left: 0.5em; bottom: 0.5em;display:inline;"
          ),div(h3(strong("Statistical GC content")), h4("Count the GC content of each sequence in the fastafile"),style="text-align: center;display:inline;",
		  h4("Required field",style="text-align: left;font-weight: 1000;color:red"),
		  div(id = "app7_fileupload",
                            tags$style("
                            .btn-file {
                 ...
                 style = "color: white; 
					   ...
					   vertical-align:middle;
					   "), target = "_blank",
                            href = "app7/sequence.fasta"),a(h4("Download sample data", class = "action-button", 
                 style = "color: white; 
                       background-color: #53d362; 
                       height: 40px;
                       width: 90px;
...
div(textAreaInput("app7_email", "","", width = "100%",height="40px"),style="text-align: center;font-weight: 1000;color:red"),
							actionButton(
                 inputId = "send_button",
                 label = "Submit",
                 styleclass = "info",
