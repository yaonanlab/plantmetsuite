...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app16_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app16_inputFile1$datapath
    ...
    if(!is.null(input$app16_email)&(input$app16_email)!=""){
	app16_email <- as.character(input$app16_email)
	}else{
	app16_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app16",mytaskid=taskidnow,myemail=app16_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app16_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app16/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
