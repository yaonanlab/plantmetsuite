...
taskidnow<-args[1]
app31_select1<-as.character(args[2])
app31_email<-args[3]
taskidnow<-as.character(taskidnow)
app31_email <- as.character(app31_email)


	if(!is.null(app31_select1)&(app31_select1)!=""){
	app31_select1 <- as.character(app31_select1)
	}else{
	app31_select1 <-"nucl"
	}
...
mycmd1<-paste("/srv/myapps/keyandaydayup/scripts/app31/fasta_no_blank.pl ",filename1,"|head -n 200 > ",outname1)
if(app31_select1=="nucl"){
...
mycmd4<-paste("/srv/R/bin/Rscript /srv/myapps/keyandaydayup/scripts/app31/anno.r  -a /srv/myapps/keyandaydayup/database/swissprot/sprot.anno -i ",outname3," -o ",outname4,sep="")
system(mycmd1)
system(mycmd2)
system(mycmd3)
system(mycmd4)
...
if(app31_email!=""&("TRUE"%in%grepl("@",app31_email))){
setwd("/srv/myapps/keyandaydayup/")
tar(file.path("userdata",taskidnow,"result.zip"),files=file.path("userdata",taskidnow,"result"))
attach.file <- file.path("/srv/myapps/keyandaydayup/userdata",taskidnow,"result.zip")
sendmail(to=as.character(app31_email),attach=T,attach.file=attach.file)
}
