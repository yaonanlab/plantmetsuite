...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app31_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app31_inputFile1$datapath
	...
	app31_select1 <- as.character(input$app31_select1)
    if(!is.null(input$app31_email)&(input$app31_email)!=""){
	app31_email <- as.character(input$app31_email)
	}else{
	app31_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app31",mytaskid=taskidnow,myemail=app31_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app31_select1,app31_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app31/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
