...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app30_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app30_inputFile1$datapath
	...
	app30_select1 <- as.character(input$app30_select1)
    if(!is.null(input$app30_email)&(input$app30_email)!=""){
	app30_email <- as.character(input$app30_email)
	}else{
	app30_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app30",mytaskid=taskidnow,myemail=app30_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app30_select1,app30_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app30/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
