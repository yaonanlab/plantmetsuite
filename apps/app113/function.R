...
taskidnow<-args[1]
app113_select1<-as.logical(args[2])#单样本、配对，TRUE/FALSE
app113_select2<-args[3]#检验值，p
app113_text1<-as.numeric(args[4])#检验值的阈值
app113_text2<-as.numeric(args[5])#log2FC阈值
app113_email<-args[6]
...
#写入pvalue
  if(firstgrouplength<2|secondgrouplength<2){
    ...
    pvalue<-wilcox.test(chartA[,i],chartB[,i], paired=app113_select1)
...
for (i in 1:nrow(data)){
	if (data$log2foldchange[i]>app113_text2){
		data$up_down[i]<-"up"
	}
	if (data$log2foldchange[i]<(-app113_text2)){
		data$up_down[i]<-"down"
	}
	if ((data$log2foldchange[i]<app113_text2)&(data$log2foldchange[i]>(-app113_text2))){
		data$up_down[i]<-"-"
	}
	if (app113_select2=="p"){
	  if(firstgrouplength<2|secondgrouplength<2){
		if (data$pvalue[i]<2&(abs(data$log2foldchange[i])>app113_text2)){ #绝对值
		  data$sig[i]<-"yes"} 
		else {
		  data$sig[i]<-"no"
		}
	  ...
	  if(firstgrouplength>=2&secondgrouplength>=2){
		if (data$pvalue[i]<app113_text1){#添加选项，选择Pvalue或者Qvalue
			data$sig[i]<-"yes"
		} else {
			data$sig[i]<-"no"
		}
	...
	if (app113_select2=="q"){
	  if(firstgrouplength<2|secondgrouplength<2){
		if (data$qvalue[i]<2&(abs(data$log2foldchange[i])>app113_text2)){ #绝对值
		  data$sig[i]<-"yes"} 
		else {
		  data$sig[i]<-"no"
		}
	  ...
	  if(firstgrouplength>=2&secondgrouplength>=2){
		if (data$qvalue[i]<app113_text1){#添加选项，选择Pvalue或者Qvalue
			data$sig[i]<-"yes"
		} else {
			data$sig[i]<-"no"
		}
...
if(app113_email!=""&("TRUE"%in%grepl("@",app113_email))){
setwd("/srv/myapps/keyandaydayup/")
tar(file.path("userdata",taskidnow,"result.zip"),files=file.path("userdata",taskidnow,"result"))
attach.file <- file.path("/srv/myapps/keyandaydayup/userdata",taskidnow,"result.zip")
sendmail(to=as.character(app113_email),attach=T,attach.file=attach.file)
}
