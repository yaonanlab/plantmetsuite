...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app113_inputFile1)&!is.null(input$app113_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app113_inputFile1$datapath
	...
	temp.file2 <- input$app113_inputFile2$datapath
	...
	if(!is.null(input$app113_text1)&(input$app113_text1)!=""){
	app113_text1 <- as.numeric(input$app113_text1)
	}else{
	app113_text1 <-0.05
	}
	if(!is.null(input$app113_text2)&(input$app113_text2)!=""){
	app113_text2 <- as.numeric(input$app113_text2)
	}else{
	app113_text2 <-1
	}
	if(!is.null(input$app113_select1)&(input$app113_select1)!=""){
	app113_select1 <- as.logical(input$app113_select1)
	}else{
	app113_select1 <-TRUE
	}
	if(!is.null(input$app113_select2)&(input$app113_select2)!=""){
	app113_select2 <- as.character(input$app113_select2)
	}else{
	app113_select2 <-"p"
	}
    if(!is.null(input$app113_email)&(input$app113_email)!=""){
	app113_email <- as.character(input$app113_email)
	}else{
	app113_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app113",mytaskid=taskidnow,myemail=app113_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}

mycmdfirst<-paste(taskidnow,app113_select1,app113_select2,app113_text1,app113_text2,app113_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app113/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
