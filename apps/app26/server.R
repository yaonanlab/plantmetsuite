...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app26_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app26_inputFile1$datapath
    ...
    if(!is.null(input$app26_email)&(input$app26_email)!=""){
	app26_email <- as.character(input$app26_email)
	}else{
	app26_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app26",mytaskid=taskidnow,myemail=app26_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app26_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app26/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
