...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app12_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app12_inputFile1$datapath
	...
	app12_text1 <- as.numeric(input$app12_text1)
	app12_text2 <- as.numeric(input$app12_text2)
    if(!is.null(input$app12_email)&(input$app12_email)!=""){
	app12_email <- as.character(input$app12_email)
	}else{
	app12_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app12",mytaskid=taskidnow,myemail=app12_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app12_text1,app12_text2,app12_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app12/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
