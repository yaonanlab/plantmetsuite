...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app67_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app67_inputFile1$datapath
    ...
    if(!is.null(input$app67_email)&(input$app67_email)!=""){
	app67_email <- as.character(input$app67_email)
	}else{
	app67_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app67",mytaskid=taskidnow,myemail=app67_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app67_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app67/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
