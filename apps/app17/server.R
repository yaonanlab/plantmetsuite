...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app17_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app17_inputFile1$datapath
    ...
    if(!is.null(input$app17_email)&(input$app17_email)!=""){
	app17_email <- as.character(input$app17_email)
	}else{
	app17_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app17",mytaskid=taskidnow,myemail=app17_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app17_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app17/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
