...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app33_inputFile1)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app33_inputFile1$datapath
    ...
    if(!is.null(input$app33_email)&(input$app33_email)!=""){
	app33_email <- as.character(input$app33_email)
	}else{
	app33_email <-"no"
	}
	ip <- "no"
	#if(!is.null(ip)&ip!=""&!is.na(ip)){
	#ipresult<-data.frame(myip=ip,myapp="app33",mytaskid=taskidnow,myemail=app33_email)
	#write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	#}
mycmdfirst<-paste(taskidnow,app33_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app33/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
