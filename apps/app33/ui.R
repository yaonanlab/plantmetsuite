appUI<-function(input,output){
    ...
    div(
      ...
      tabBox(
        ...
        height = NULL,
        tabPanel(
          title = img(src = 'app33.png', height = '40px', width = '40px',style="display: inline;"),
          div(
            style = "position: absolute; left: 0.5em; bottom: 0.5em;display:inline;"
          ),div(h3(strong("VCF extraction of SNPs")), h4("Extraction of genomically inconsistent SNP loci from VCFfile"),style="text-align: center;display:inline;",
		  h4("Required field",style="text-align: left;font-weight: 1000;color:red"),
		  div(id = "app33_fileupload",
                            tags$style("
                            .btn-file {
							...
							"),br(),h4("Select the data to be uploaded：vcf、VCF、txt",style="text-align: left;font-weight: 1000;")
							,
							fluidRow(column(width=5,fileInput(
                                inputId="app33_inputFile1",
                                label = "",
                                accept = c('.vcf',
                                           '.VCF',
                                           '.txt')
                 ...
                 style = "color: white; 
					   ...
					   vertical-align:middle;
					   "), target = "_blank",
                            href = "app33/data.txt"),a(h4("Download sample data", class = "action-button", 
                 style = "color: white; 
                       background-color: #53d362; 
                       height: 40px;
                       width: 90px;
...
div(textAreaInput("app33_email", "","", width = "100%",height="40px"),style="text-align: center;font-weight: 1000;color:red"),
							actionButton(
                 inputId = "send_button",
                 label = "Submit",
                 styleclass = "info",
