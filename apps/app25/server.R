...
job.path <- reactiveValues(data = NULL)
    if(!is.null(input$app25_inputFile1)&!is.null(input$app25_inputFile2)){
	load("userdata/jobid")
	...
	temp.file1 <- input$app25_inputFile1$datapath
	...
	temp.file2 <- input$app25_inputFile2$datapath
    ...
    if(!is.null(input$app25_email)&(input$app25_email)!=""){
	app25_email <- as.character(input$app25_email)
	}else{
	app25_email <-"no"
	}
	ip <- "no"
	if(!is.null(ip)&ip!=""&!is.na(ip)){
	ipresult<-data.frame(myip=ip,myapp="app25",mytaskid=taskidnow,myemail=app25_email)
	write.table(ipresult, "userdata/userip.xls",append=T,row.names=F,col.names=F,sep="\t",quote=F)
	}
mycmdfirst<-paste(taskidnow,app25_email,sep="' '")
mycmdsecond<-paste("nohup ","/srv/R/bin/Rscript /srv/myapps/keyandaydayup/apps/app25/function.R ","'",mycmdfirst,"'"," 1>/dev/null 2>/dev/null &",sep="")
system(mycmdsecond)
